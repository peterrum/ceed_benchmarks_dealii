        ...
        if (p == Utilities::MPI::this_mpi_process(MPI_COMM_WORLD))
          {
            ...
            for (unsigned int i = 0;
                 i < matrix_free->get_dof_info().vector_zero_range_list_index.size() - 1;
                 ++i)
              {
                for (unsigned int j = matrix_free->get_dof_info().vector_zero_range_list_index[i];
                     j < matrix_free->get_dof_info().vector_zero_range_list_index[i + 1];
                     ++j)
                  {
            ...
            std::cout << "pre list: ";
            for (unsigned int i = 0;
                 i < matrix_free->get_dof_info().cell_loop_pre_list_index.size() - 1;
                 ++i)
              {
                for (unsigned int j = matrix_free->get_dof_info().cell_loop_pre_list_index[i];
                     j < matrix_free->get_dof_info().cell_loop_pre_list_index[i + 1];
                     ++j)
                  {
            ...
            std::cout << "post list: ";
            for (unsigned int i = 0;
                 i < matrix_free->get_dof_info().cell_loop_post_list_index.size() - 1;
                 ++i)
              {
                for (unsigned int j = matrix_free->get_dof_info().cell_loop_post_list_index[i];
                     j < matrix_free->get_dof_info().cell_loop_post_list_index[i + 1];
                     ++j)
                  {
  ...
  if (s_in < 1)
    {
      if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
        std::cout
          << " p |  q | n_elements |      n_dofs |     time/it |op_dofs/s/it | opt_time/it | op3_time/it | opm_time/it | CG_its | time/matvec"
          << std::endl;
      unsigned int s =
        std::max(3U,
